...
const DetalleCredito = () => {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        showError("Ocurrió un error al eliminar el crédito");
      }
    }
  };
  ...
  const handleRegisterPayment = () => {
    ...
    setCredit({
      ...credit,
      pendingAmount: credit.pendingAmount - amount,
      paidAmount: credit.paidAmount + amount,
      paymentHistory: [
        { id: credit.paymentHistory.length + 1, date: paymentDate, amount: amount, status: "Pagado" },
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: "#F3F4F6" }}>
        ...
        <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ width: "60px", height: "60px", border: "3px solid #E5E7EB", borderTopColor: "#8C7354", borderRadius: "50%", animation: "spin 1s linear infinite", margin: "0 auto 16px auto" }}></div>
            <p style={{ fontSize: "1.1rem", color: "#6B7280" }}>Cargando información del crédito...</p>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#F3F4F6" }}>
      ...
      <div style={{
        ...
        <div style={{ padding: "28px 20px", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ width: "36px", height: "36px", background: "#8C7354", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ color: "white", fontWeight: "bold", fontSize: "22px" }}>C</span>
            </div>
            <div>
              <span style={{ color: "#8C7354", fontWeight: "bold", fontSize: "1.6rem" }}>COMFÍA</span>
              <p style={{ fontSize: "1rem", color: "#6B7280", marginTop: "2px" }}>Sistema de Crédito</p>
            </div>
          </div>
        </div>
        ...
        <nav style={{ flex: 1, padding: "20px 12px" }}>
          {menuItems.map((item) => (
            <div key={item.name} onClick={() => navigate(item.path)}
              style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 16px", marginBottom: "4px", borderRadius: "10px", cursor: "pointer", background: item.active ? "#8C7354" : "transparent", color: item.active ? "white" : "#6B7280" }}>
              <MaterialIcon name={item.icon} style={{ color: item.active ? "white" : "#6B7280" }} />
              <span style={{ fontSize: "1.5rem", fontWeight: 500 }}>{item.name}</span>
            </div>
          ))}
        </nav>
        ...
        <div style={{ padding: "20px", borderTop: "1px solid rgba(0,0,0,0.05)" }}>
          <div onClick={() => navigate("/perfil")}
            style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px", cursor: "pointer", borderRadius: "10px", padding: "4px 8px", transition: "background 0.2s" }}
            onMouseEnter={(e) => (e.currentTarget.style.background = "#E8E0A0")}
            onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
            <div style={{ width: "40px", height: "40px", background: "#8C7354", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "bold", fontSize: "22px" }}>
              {user?.name?.charAt(0) || "A"}
            </div>
            <div>
              <p style={{ color: "#8C7354", fontSize: "1.4rem", fontWeight: 600 }}>{user?.name || "Admin User"}</p>
              <p style={{ color: "#9CA3AF", fontSize: "1.1rem" }}>Administrador</p>
            </div>
          </div>
          <button onClick={handleLogout} style={{ width: "100%", display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", background: "transparent", border: "1px solid #E5E7EB", borderRadius: "10px", color: "#6B7280", fontSize: "1rem", cursor: "pointer" }}>
            <MaterialIcon name="logout" style={{ fontSize: "22px" }} /> Cerrar Sesión
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: "32px", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
          <div>
            <h1 style={{ fontSize: "3rem", fontWeight: 700, color: "#1F2937" }}>Detalle del Crédito</h1>
            <p style={{ fontSize: "1.5rem", color: "#6B7280", marginTop: "4px" }}>Información completa del crédito #{credit.id}</p>
          </div>
          <button onClick={() => navigate("/creditos")} style={{ display: "flex", alignItems: "center", gap: "8px", background: "white", border: "1px solid #E5E7EB", borderRadius: "8px", padding: "8px 16px", fontSize: "1.2rem", cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", marginBottom: "32px" }}>
          ...
          <div style={{ background: "white", borderRadius: "20px", padding: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", border: "1px solid #F3F4F6" }}>
            <h2 style={{ fontSize: "1.5rem", fontWeight: 600, color: "#8C7354", marginBottom: "24px", paddingBottom: "12px", borderBottom: "2px solid #F4ECA6" }}>
              <MaterialIcon name="info" style={{ fontSize: "22px", marginRight: "8px", verticalAlign: "middle" }} />
              Información General
            </h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
              <div>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>CLIENTE</p>
                <p style={{ fontSize: "1.4rem", fontWeight: 500, color: "#1F2937", marginBottom: "20px" }}>{credit.client}</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>CÉDULA</p>
                <p style={{ fontSize: "1.4rem", color: "#1F2937", marginBottom: "20px" }}>{credit.document}</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>FECHA SOLICITUD</p>
                <p style={{ fontSize: "1.4rem", color: "#1F2937" }}>{credit.date}</p>
              </div>
              <div>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>TIPO</p>
                <p style={{ fontSize: "1.4rem", fontWeight: 500, color: "#1F2937", marginBottom: "20px" }}>{credit.type}</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>ESTADO</p>
                <p style={{ marginBottom: "20px" }}>
                  <span style={{ padding: "5px 14px", borderRadius: "20px", fontSize: "1.2rem", fontWeight: 600, background: statusStyle.bg, color: statusStyle.color }}>
                    {statusStyle.text}
                  </span>
                </p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>PRÓXIMO PAGO</p>
                <p style={{ fontSize: "1.4rem", color: "#1F2937" }}>{credit.nextPayment}</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: "white", borderRadius: "20px", padding: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", border: "1px solid #F3F4F6" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
              <div>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>MONTO TOTAL</p>
                <p style={{ fontSize: "1.5rem", fontWeight: 700, color: "#1F2937", marginBottom: "20px" }}>{formatCurrency(credit.amount)}</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>SALDO PENDIENTE</p>
                <p style={{ fontSize: "1.4rem", fontWeight: 600, color: "#EF4444", marginBottom: "20px" }}>{formatCurrency(credit.pendingAmount)}</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>TASA INTERÉS</p>
                <p style={{ fontSize: "1.4rem", color: "#1F2937" }}>{credit.interest}% anual</p>
              </div>
              <div>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>TOTAL PAGADO</p>
                <p style={{ fontSize: "1.4rem", fontWeight: 600, color: "#10B981", marginBottom: "20px" }}>{formatCurrency(credit.paidAmount)}</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>PLAZO</p>
                <p style={{ fontSize: "1.4rem", color: "#1F2937", marginBottom: "20px" }}>{credit.term} meses</p>
                <p style={{ fontSize: "1.2rem", color: "#9CA3AF", marginBottom: "6px" }}>VALOR CUOTA</p>
                <p style={{ fontSize: "1.4rem", color: "#1F2937" }}>{formatCurrency(Math.round(credit.amount / credit.term))}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "white", borderRadius: "20px", padding: "28px", marginBottom: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", border: "1px solid #F3F4F6" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <h2 style={{ fontSize: "1.5rem", fontWeight: 600, color: "#8C7354" }}>
            ...
            {credit.status !== "Pagado" && credit.pendingAmount > 0 && (
              <button onClick={() => setShowPaymentModal(true)} style={{ background: "#8C7354", color: "white", border: "none", padding: "8px 20px", borderRadius: "8px", fontSize: "1rem", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #E5E7EB" }}>
                  <th style={{ padding: "14px", textAlign: "left", fontSize: "1.2rem", color: "#6B7280", fontWeight: 600 }}>#</th>
                  <th style={{ padding: "14px", textAlign: "left", fontSize: "1.2rem", color: "#6B7280", fontWeight: 600 }}>FECHA</th>
                  <th style={{ padding: "14px", textAlign: "left", fontSize: "1.2rem", color: "#6B7280", fontWeight: 600 }}>MONTO</th>
                  <th style={{ padding: "14px", textAlign: "left", fontSize: "1.2rem", color: "#6B7280", fontWeight: 600 }}>ESTADO</th>
                </tr>
              </thead>
              <tbody>
                {credit.paymentHistory.map((payment, index) => {
                  ...
                  return (
                    <tr key={payment.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                      <td style={{ padding: "12px", fontSize: "1.2rem", color: "#1F2937" }}>{index + 1}</td>
                      <td style={{ padding: "12px", fontSize: "1.2rem", color: "#6B7280" }}>{payment.date}</td>
                      <td style={{ padding: "12px", fontSize: "1.2rem", fontWeight: 500, color: "#1F2937" }}>{formatCurrency(payment.amount)}</td>
        ...
        <div style={{ background: "#FFF9E6", borderRadius: "20px", padding: "24px", marginBottom: "24px", border: "1px solid #FDE68A" }}>
          <h3 style={{ fontSize: "1.2rem", fontWeight: 600, color: "#8C7354", marginBottom: "12px" }}>
            <MaterialIcon name="description" style={{ fontSize: "20px", marginRight: "8px", verticalAlign: "middle" }} />
            Notas Adicionales
          </h3>
          <p style={{ fontSize: "1.2rem", color: "#6B7280", lineHeight: 1.5 }}>{credit.notes}</p>
        </div>

        {/* Botones de Acción */}
        <div style={{ display: "flex", gap: "16px", justifyContent: "flex-end" }}>
          <button onClick={() => navigate(`/creditos/editar/${credit.id}`)} style={{ display: "flex", alignItems: "center", gap: "8px", background: "white", border: "1px solid #8C7354", color: "#8C7354", padding: "10px 24px", borderRadius: "8px", fontSize: "1.2rem", fontWeight: 500, cursor: "pointer" }}>
            <MaterialIcon name="edit" style={{ fontSize: "20px" }} /> Editar Crédito
          </button>
          <button onClick={handleDelete} style={{ display: "flex", alignItems: "center", gap: "8px", background: "#EF4444", color: "white", border: "none", padding: "10px 24px", borderRadius: "8px", fontSize: "1.2rem", fontWeight: 600, cursor: "pointer" }}>
      ...
      {showPaymentModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowPaymentModal(false)}>
          <div style={{ background: "white", borderRadius: "20px", width: "480px", maxWidth: "90%", padding: "32px", boxShadow: "0 20px 35px rgba(0,0,0,0.15)" }} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ fontSize: "1.8rem", fontWeight: 700, color: "#1F2937", marginBottom: "24px" }}>Registrar Pago</h2>
